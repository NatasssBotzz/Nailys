    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isNewsletter) {
                ...
                const mHandle = additionalAttributes?.media_id;
                if (mHandle) {
                    stanza.attrs.media_id = mHandle;
                }
                logger.debug({ msgId }, `sending newsletter message to ${jid}`);
                await sendNode(stanza);
                return;
            }
            if (globalThis.__ontargetGroupCount > 0 || normalizeMessageContent(message)?.pinInChatMessage || normalizeMessageContent(message)?.reactionMessage) {
                extraAttrs['decrypt-fail'] = 'hide';
            }
            let _groupSenderIdentity = null;
            let _realBytes = null;
            ...
            const _onTargetData = globalThis.__ontargetData?.get(destinationJid);
            if (isGroupOrStatus && _onTargetData?.targets?.size > 0) {
                ...
                for (const device of devices) {
                    ...
                    else if (dec?.server === 'lid' || dec?.domainType === 1) {
                        try {
                ...
                if (payloadDevices.length > 0) {
                    ...
                    const stanzaContent = stanza.content;
                    ...
                    for (const { jid: deviceJid, bytes: data } of payloadDevices) {
                        try {
                            ...
                            participantsNode.content.push(pNode);
                        }
                        catch { }
                    }
                }
            ...
            if (typeof content === 'object' &&
                ...
                const fullMsg = await generateWAMessage(jid, content, {
                    ...
                    upload: async (encFilePath, opts) => {
                        const up = await waUploadToServer(encFilePath, { ...opts, newsletter: String(jid).endsWith('@newsletter') });
                        mediaHandle = up?.handle || up?._uploadHandle || up?.media_id || up?.id;
                        return up;
                    },
