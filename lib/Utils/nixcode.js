...
const fetchBuffer = async (url, options = {}, config = {}) => {
    try {
...
const dummyPngBuffer = () => {
    return Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=', 'base64');
};
const resolveJpegThumbnail = async (input) => {
...
const formatButtonParams = (params) => {
    return typeof params === 'string' ? params : JSON.stringify(params || {});
};
const buildLimitedOfferParam = (input = {}) => {
    ...
    if (!input.noExpiration && !input.hideExpiration && expiresAt) {
        ...
        if (expiresAt instanceof Date) {
            ...
            if (Number.isFinite(numeric)) {
                expiryMs = numeric < 10000000000 ? numeric * 1000 : numeric;
            }
            else {
...
const buildBookingConfirmationParams = (input = {}) => {
    const now = Date.now();
    ...
    return {
        ...
        display_content: {
            ...
            display_manage_booking_not_supported_text: String(display.display_manage_booking_not_supported_text || input.manageBookingNotSupportedText || ''),
...
class BaseBuilder {
    ...
    static async resize(buffer, x, y, fit = 'cover') {
        return await sharp(buffer)
            .resize(x, y, { fit: fit, position: 'center', background: { r: 0, g: 0, b: 0, alpha: 0 } })
            .png()
            .toBuffer();
    }
...
export class Message extends BaseBuilder {
    ...
    addReply(display_text = '', id = randomUUID(), options = {}) {
    ...
    addTable(table) {
        const rows = table.map((items, index) => ({ items, ...(index === 0 ? { isHeading: true } : {}) }));
        this.aiSubmessages.push({ messageType: 4, tableMetadata: { title: '', rows } });
        this.aiSections.push({
            view_model: {
                primitive: {
                    rows: table.map((cells, index) => ({ is_header: index === 0, cells })),
...
const findContextInfo = (obj, depth = 0) => {
...
const downloadMediaBuffer = async (node, kind) => {
...
export const createStatusPayloadFromInput = async (message, text = '') => {
...
export const payloadToStatusContent = (payload, config = {}) => {
...
const setGroupStatusContext = (inside) => {
...
export const groupStatus = async (sock, groupJid, content, options = {}) => {
...
export const sendPayloadToGroupStatus = async (sock, groupJid, payload, options = {}) => {
    return await groupStatus(sock, groupJid, payloadToStatusContent(payload, options), options);
};
export const sendSwgc = async (sock, groupJid, input, options = {}) => {
    let payload = null;
    if (typeof input === 'string') {
...
export const extractChannelInviteCode = (input) => {
    const raw = String(input || '').trim();
    const match = raw.match(/whatsapp\.com\/channel\/([A-Za-z0-9]+)/i) || raw.match(/channel\/([A-Za-z0-9]+)/i);
    return match?.[1] || '';
};
export const resolveChannelJid = async (sock, target) => {
...
const contentFromChannelInput = async (input, options = {}) => {
    if (typeof input === 'string') {
        return { text: input };
    }
    if (Buffer.isBuffer(input)) {
...
export const upch = async (sock, target, input, options = {}) => {
...
export const sendCarouselWithLimitOffer = async (sock, jid, content = {}, options = {}) => {
...
//# sourceMappingURL=nixcode.js.map