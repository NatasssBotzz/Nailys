...
var _Swgc_client, _Button_client, _ButtonV2_client, _Carousel_client, _AIRich_client, _StickerPack_client;
// @ts-nocheck
const VERSION = '4.4';
...
function loadSharp() {
    if (__sharpTried) {
        if (!__sharp) {
            throw new Error("Fitur ini butuh paket 'sharp' tapi tidak terpasang. " +
    ...
    try {
        if (typeof source === 'string') {
            if (/^https?:\/\//i.test(source))
                source = await fetchBuffer(source, {}, { silent: true });
...
async function sendButtons(sock, toJid, content = {}, options = {}) {
    ...
    const msg = generateWAMessageFromContent(toJid, {
        interactiveMessage: {
            header: {
                title: content.title || '',
...
function patchSwgcFile(file) {
    if (!fs.existsSync(file))
        return { file, found: false, patched: false, already: false };
    const original = fs.readFileSync(file, 'utf8');
    let source = original;
    source = source.replace(/(message\?\.associatedChildMessage)\s*\|\|\s*\n\s*message\?\.groupStatusMessage\s*\|\|\s*\n\s*message\?\.groupStatusMessageV2(\s*\);)/g, '$1$2');
    source = source
...
function patchBaileysForSwgc() {
    ...
    for (const r of touched) {
        ...
        if (r.patched)
            console.log('[SWGC PATCH] Naileys berhasil dipatch:', rel);
        else if (r.already)
            console.log('[SWGC PATCH] Naileys sudah terpatch:', rel);
        else
            console.log('[SWGC PATCH] Pola RC13 tidak berubah / tidak cocok:', rel);
    }
...
function pickMediaNode(messageObject) {
    ...
    if (type === 'videoMessage')
        return { kind: 'video', node };
    if (type === 'audioMessage')
        return { kind: 'audio', node };
    return null;
}
async function downloadMediaBuffer(node, kind) {
    if (!downloadContentFromMessage)
        throw new Error('downloadContentFromMessage tidak tersedia di Naileys');
...
async function createStatusPayloadFromInput(m, text = '') {
    ...
    if (inputCaption)
        return { type: 'text', text: inputCaption };
    return null;
}
...
function notifySwgcEcho(sock, msg = {}) {
    try {
        if (!sock?.__swgcWaiters || !msg?.key?.id)
            return false;
        if (!hasGroupStatusEnvelope(msg.message || {}))
            return false;
...
async function groupStatus(sock, jid, content, options = {}) {
    ...
    if (!jid || !String(jid).endsWith('@g.us'))
        throw new Error('Target bukan grup');
    ...
    try {
        ...
        if (echoPromise)
            ensureSwgcWaiters(sock).delete(waiterKey(jid, msg.key.id));
        throw err;
    }
    if (echoPromise) {
        ...
        if (!echo.ok) {
            ...
            const warning = 'Relay SWGC diterima server, tapi tidak ada echo groupStatusMessageV2 dari WhatsApp dalam batas waktu. ' +
            ...
            try {
                sock.logger?.warn?.({ jid, messageId: msg.key.id }, warning);
            }
            catch { }
        }
    }
...
async function sendPayloadToGroupStatus(sock, targetJid, payload, config = {}) {
    ...
    if (!content)
        throw new Error('Payload SWGC tidak valid');
...
async function resolveGroupTarget(sock, input = '', groups = []) {
    ...
    if (inviteMatch?.[1]) {
        ...
        if (info?.id)
            return String(info.id).endsWith('@g.us') ? info.id : `${info.id}@g.us`;
    }
...
function splitTargetAndContent(text = '') {
    ...
    if (parts.length < 2)
        return { targetText: '', contentText: raw };
    return {
...
async function sendSwgc(sock, targetJid, input, options = {}) {
    ...
    if (!payload)
        throw new Error('Input SWGC tidak valid');
...
class Swgc {
    constructor(client) {
        _Swgc_client.set(this, void 0);
        if (!client)
            throw new Error('Socket is required');
        __classPrivateFieldSet(this, _Swgc_client, client, "f");
        this._target = '';
        this._payload = null;
        this._options = {};
    }
    ...
    swgc(input, targetJid = '', options = {}) {
        if (targetJid)
            this.target(targetJid);
        this._options = { ...this._options, ...options };
        if (typeof input === 'string')
            return this.text(input);
        if (Buffer.isBuffer(input))
            return this.media(input, options.type || options.kind || 'image', options);
        if (input?.type) {
            this._payload = input;
            return this;
        }
    ...
    async send(targetJid = '', options = {}) {
        ...
        return await sendSwgc(__classPrivateFieldGet(this, _Swgc_client, "f"), target, this._payload, opts);
    }
}
_Swgc_client = new WeakMap();
class BaseBuilder {
    constructor() {
        this._title = '';
        this._subtitle = '';
...
function buildBookingConfirmationParams(input = {}) {
    ...
    const now = Date.now();
    const start = input.start_datetime || input.startDatetime || input.start || new Date(now + 60000).toISOString();
    const end = input.end_datetime || input.endDatetime || input.end || new Date(now + 660000).toISOString();
    return {
...
class Button extends BaseBuilder {
    ...
    async toCard() {
        const preparedMedia = this._data
            ? await prepareWAMessageMedia(this._data, { upload: __classPrivateFieldGet(this, _Button_client, "f").waUploadToServer }).catch((error) => {
        ...
        if (preparedMedia.documentMessage) {
            ...
            if (Buffer.isBuffer(jpegThumbnail) && jpegThumbnail.length)
                preparedMedia.documentMessage.jpegThumbnail = jpegThumbnail;
            if (this._data?.fileLength)
                preparedMedia.documentMessage.fileLength = this._data.fileLength;
            if (this._data?.pageCount)
                preparedMedia.documentMessage.pageCount = this._data.pageCount;
            if (this._data?.title)
                preparedMedia.documentMessage.title = this._data.title;
            if (this._data?.fileName)
                preparedMedia.documentMessage.fileName = this._data.fileName;
            if (this._data?.caption)
                preparedMedia.documentMessage.caption = this._data.caption;
...
class ButtonV2 extends BaseBuilder {
    ...
    async buildNative(jid, { ...options } = {}) {
        const header = {
            ...
            subtitle: this._subtitle,
            hasMediaAttachment: !!this._data,
            ...(this._data ? await prepareWAMessageMedia(this._data, { upload: __classPrivateFieldGet(this, _ButtonV2_client, "f").waUploadToServer }) : {})
        };
        return generateWAMessageFromContent(jid, {
            ...this._extraPayload,
            interactiveMessage: {
    ...
    async build(jid, { ...options } = {}) {
        if (this._nativeButtons.length)
            return await this.buildNative(jid, options);
...
function buildLimitedOfferParam(input = {}) {
    ...
    if (offer.thumbnailUrl)
        out.thumbnail_url = offer.thumbnailUrl;
    if (!offer.noExpiration && offer.expiresAt) {
        let expiryMs = 0;
        if (offer.expiresAt instanceof Date)
            expiryMs = offer.expiresAt.getTime();
        else if (typeof offer.expiresAt === 'number')
            expiryMs = offer.expiresAt < 10000000000 ? offer.expiresAt * 1000 : offer.expiresAt;
...
function mergeLimitedOfferParams(messageParamsJson, limitOffer) {
    ...
    if (limitOffer) {
        ...
        if (offer)
            params.limited_time_offer = offer;
    }
    return params;
}
...
function cleanCarouselCardTexts(card = {}) {
    if (card.body && typeof card.body === 'object' && !carouselHasText(card.body.text || ''))
        delete card.body;
    if (card.footer && typeof card.footer === 'object' && !carouselHasText(card.footer.text || ''))
        delete card.footer;
    if (card.header && typeof card.header === 'object') {
        if (!carouselHasText(card.header.title || ''))
            delete card.header.title;
        if (!carouselHasText(card.header.subtitle || ''))
            delete card.header.subtitle;
    }
    return card;
}
function injectLimitedOfferToCard(card = {}, limitOffer = null) {
    ...
    if (cardOffer || card?.nativeFlowMessage?.messageParamsJson) {
        ...
        if (json)
            card.nativeFlowMessage.messageParamsJson = json;
        else
            delete card.nativeFlowMessage.messageParamsJson;
    }
...
async function buildCarouselCardFromContent(sock, card = {}, options = {}) {
    ...
    if (card.video || card.header?.video) {
        ...
        if (Buffer.isBuffer(doc) || typeof doc === 'string')
            builder.setDocument(doc, card.documentOptions || {});
        else if (doc && typeof doc === 'object')
            builder.setDocument(doc.buffer || doc.url || Buffer.from(''), doc);
    }
    else if (headerType) {
...
async function sendCarouselWithLimitOffer(sock, jid, content = {}, options = {}) {
    ...
    for (const card of content.cards) {
        ...
        for (const rawButton of buttons) {
            ...
            if (btn)
                builder.addButton(btn.name, btn.buttonParamsJson);
        }
        ...
        if (offer && !buttons.length) {
            const normalized = normalizeLimitOfferInput(offer);
            builder.addUrl(normalized?.text || '\u200e', normalized?.url || 'https://example.com', false);
        }
...
class AIRich {
    ...
    addLatex(text, items = []) {
        ...
        if (!Array.isArray(items))
            throw new TypeError('Latex items must be an array');
...
function patchDefaultsCode(code) {
    ...
    if (!out.includes("'sticker-pack': '/mms/sticker-pack'")) {
        const next = out.replace(/sticker:\s*['"]\/mms\/image['"],\s*\n(\s*)['"]thumbnail-link['"]:/, match => match.replace(/\n(\s*)['"]thumbnail-link['"]:/, "\n$1'sticker-pack': '/mms/sticker-pack',\n$1'thumbnail-link':"));
        if (next !== out) {
            out = next;
            changed = true;
        }
    ...
    if (!out.includes("'sticker-pack': 'Sticker Pack'")) {
        const next = out.replace(/sticker:\s*['"]Image['"],\s*\n(\s*)video:/, match => match.replace(/\n(\s*)video:/, "\n$1'sticker-pack': 'Sticker Pack',\n$1video:"));
        if (next !== out) {
            out = next;
            changed = true;
        }
...
function patchBaileysStickerPackMediaRc13_FORCE() {
    ...
    for (const file of candidates) {
        ...
        const alreadyPatched = before.includes("'sticker-pack': '/mms/sticker-pack'") &&
        ...
        const ok = after.includes("'sticker-pack': '/mms/sticker-pack'") &&
...
function ensureStickerPackSock(sock) {
    ...
    if (typeof sock.relayMessage !== 'function')
        throw new Error('sock.relayMessage is required');
    return sock;
}
function normalizeStickerPackInput(input = {}) {
    const src = input?.stickerPack && typeof input.stickerPack === 'object'
        ? input.stickerPack
        : input;
    const packname = String(src?.packname ||
        src?.name ||
        src?.title ||
        'Sticker Pack').trim() || 'Sticker Pack';
    const author = String(src?.author ||
        src?.publisher ||
        '').trim();
...
async function uploadStickerPackBlob(buffer, ctx = {}, fileName = 'sticker-pack.zip') {
    ...
    const { mediaKey, encFilePath, originalFilePath, fileEncSha256, fileSha256, fileLength } = await encryptedStream(buffer, 'sticker-pack', {
        logger: ctx?.logger,
    ...
    try {
        ...
        try {
            if (encFilePath && fs.existsSync(encFilePath))
...
async function buildSingleStickerPackMessage(stickerPack, ctx = {}) {
    ...
    const packMedia = await uploadStickerPackBlob(zipBuffer, {
        ...ctx,
        upload,
        sock: ctx?.sock
    }, `${stickerPackId}.zip`);
...
async function prepareStickerPackMessage(arg1, arg2 = {}, arg3 = {}) {
    const usingSockFirst = arg1 && typeof arg1 === 'object' && (typeof arg1.waUploadToServer === 'function' ||
...
async function sendStickerPack(sock, jid, stickerPack, options = {}) {
    ...
    const userJid = (options?.userJid ||
        sock?.user?.id ||
        sock?.authState?.creds?.me?.id ||
        '0@s.whatsapp.net');
    const sendSingle = async (payload) => {
        const msg = generateWAMessageFromContent(jid, { stickerPackMessage: payload }, {
            userJid,
            quoted: options?.quoted,
...
class StickerPack extends BaseBuilder {
    constructor(client) {
        super();
        _StickerPack_client.set(this, void 0);
        if (!client)
            throw new Error('Socket is required');
        __classPrivateFieldSet(this, _StickerPack_client, client, "f");
        this._pack = {
    ...
    addStickers(stickers = []) {
        if (!Array.isArray(stickers))
            throw new TypeError('Stickers must be an array');
    ...
    async prepare(options = {}) { return await prepareStickerPackMessage(__classPrivateFieldGet(this, _StickerPack_client, "f"), this.toJSON(), options); }
    async send(jid, options = {}) { return await sendStickerPack(__classPrivateFieldGet(this, _StickerPack_client, "f"), jid, this.toJSON(), options); }
}
_StickerPack_client = new WeakMap();
