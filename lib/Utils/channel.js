...
import { S_WHATSAPP_NET, getBinaryNodeChild, getBinaryNodeChildren, getAllBinaryNodeChildren } from '../WABinary/index.js';
...
function _ensureSock(sock) {
    ...
    if (typeof sock.query !== 'function')
        throw new Error('sock.query is required');
    if (typeof sock.relayMessage !== 'function')
        throw new Error('sock.relayMessage is required');
    if (typeof sock.sendMessage !== 'function')
        throw new Error('sock.sendMessage is required');
    if (typeof sock.waUploadToServer !== 'function')
        throw new Error('sock.waUploadToServer is required');
    return sock;
}
async function _maybeDecryptNewsletterMessage(sock, messageNode) {
    ...
    if (!decryptMessageNode || !authState || !signalRepository || !myId)
        return null;
    const { fullMessage, decrypt } = await decryptMessageNode(messageNode, myId, myLid, signalRepository, sock?.logger);
    await decrypt();
    return fullMessage;
}
...
function _pickFirstMessageNode(msg = {}) {
    ...
    if (msg.ephemeralMessage?.message)
        return _pickFirstMessageNode(msg.ephemeralMessage.message);
    if (msg.viewOnceMessage?.message)
        return _pickFirstMessageNode(msg.viewOnceMessage.message);
    if (msg.viewOnceMessageV2?.message)
        return _pickFirstMessageNode(msg.viewOnceMessageV2.message);
    if (msg.viewOnceMessageV2Extension?.message)
        return _pickFirstMessageNode(msg.viewOnceMessageV2Extension.message);
...
export function normalizeNewsletterMsg(msg = {}) {
    ...
    if (typeof msg.newsletterJid === 'undefined')
        msg.newsletterJid = remoteJid;
...
export function patchSocketNewsletterEvents(sock) {
    if (!sock?.ev || sock.__newsletterEventsPatched)
        return sock;
...
export function makeNewsletterHelper(sock) {
    ...
    const parseFetchedUpdates = async (node, type) => {
        ...
        const out = await Promise.all(items.map(async (messageNode) => {
    ...
    const sendNewsletterText = async (newsletterId, text, options = {}) => {
        ...
        if (!jid)
            throw new Error('newsletterId is required');
        const msg = generateWAMessageFromContent(jid, { extendedTextMessage: { text: text || ' ', contextInfo: options.contextInfo || {} } }, { userJid: sock.user?.id, logger: sock.logger, ...options.sendOptions });
    ...
    const relayNewsletterMessage = async (newsletterId, message, relayOptions = {}) => {
        const jid = normalizeNewsletterJid(newsletterId);
        const msgId = relayOptions.messageId ||
            (typeof sock.generateMessageTag === 'function'
                ? sock.generateMessageTag()
                : genTag());
    ...
    return {
        ...
        sendNewsletterTextLegacy: async (newsletterId, text, options = {}) => {
            const jid = normalizeNewsletterJid(newsletterId);
            const msg = generateWAMessageFromContent(jid, { extendedTextMessage: { text: text || ' ', contextInfo: options.contextInfo || {} } }, { userJid: sock.user?.id, logger: sock.logger, ...options });
            return sock.relayMessage(jid, msg.message, {
                messageId: options.messageId || msg.key.id,
                additionalAttributes: { ...(options.additionalAttributes || {}), newsletter: 'true' }
            });
...
export function patchBaileysNewsletterMediaRc13_FORCE() {
    ...
    for (const file of candidates) {
        ...
        const beforeNewsletterBlocks = (code.match(/if\s*\(\s*isNewsletter\s*\)\s*\{/g) || []).length;
        const replacement = `
if (isNewsletter) {
...
`.trim();
        const reBlockGlobal = /if\s*\(\s*isNewsletter\s*\)\s*\{[\s\S]*?\n\s*return\s*;\s*\n\s*\}/g;
        let patchedBlocks = 0;
        ...
        if (patchedBlocks === 0) {
            const reBlockFallback = /if\s*\(\s*isNewsletter\s*\)\s*\{[\s\S]*?return\s*;\s*\}/g;
        ...
        const afterNewsletterBlocks = (after.match(/if\s*\(\s*isNewsletter\s*\)\s*\{/g) || []).length;
        const safeNow = after.includes("encodeNewsletterMessage(patched)") &&
...
export function getNewsletterHelper(sock) {
    if (!sock)
        return null;
    if (sock.newsletter && typeof sock.newsletter === 'object')
        return sock.newsletter;
    try {
        const helper = makeNewsletterHelper(sock);
        sock.newsletter = helper;
        for (const [key, value] of Object.entries(helper)) {
            if (typeof value === 'function' && typeof sock[key] !== 'function')
                sock[key] = value;
        }
        return helper;
    }
...
function getNewsletterMetaFn(sock, newsletter) {
    return (sock?.newsletterMetadata ||
...
async function resolveInvite(sock, code, newsletter) {
    ...
    if (channelTargetCache.has(key))
        return channelTargetCache.get(key);
    ...
    if (!jid)
        throw new Error(lastErr?.message || 'Gagal resolve ID channel dari link.');
...
export async function resolveChannelTarget(sock, input, newsletter) {
    ...
    if (!raw)
        throw new Error('Target channel kosong.');
    if (isNewsletterJid(raw))
        return normalizeNewsletterJid(raw);
    if (/^\d+@newsletter$/i.test(raw))
        return normalizeNewsletterJid(raw);
    if (/^\d+$/.test(raw))
        return normalizeNewsletterJid(raw);
    const code = extractInviteCode(raw);
    if (code)
        return resolveInvite(sock, code, newsletter);
...
export async function resolveOwnerChannelJid(sock, newsletter) {
    ...
    if (fallback)
        return resolveChannelTarget(sock, fallback, helper);
...
function pickMedia(msgRaw = {}) {
    ...
    if (m.imageMessage)
        return { kind: 'image', msg: m.imageMessage };
    if (m.videoMessage)
        return { kind: 'video', msg: m.videoMessage };
    if (m.audioMessage)
        return { kind: 'audio', msg: m.audioMessage };
    if (m.documentMessage)
        return { kind: 'document', msg: m.documentMessage };
    if (m.stickerMessage)
        return { kind: 'sticker', msg: m.stickerMessage };
    return null;
}
...
export function getQuotedAndContextInfo(m) {
    ...
    const ci = raw?.extendedTextMessage?.contextInfo ||
...
function getMimeFromQuoted(q) {
    if (!q)
        return '';
    if (q.mimetype)
        return String(q.mimetype).toLowerCase();
    if (q.msg?.mimetype)
        return String(q.msg.mimetype).toLowerCase();
    const raw = unwrapChannelMessage(q.message || q.msg || q) || {};
...
async function sendChannelMedia(sock, channelJid, picked, buffer, caption, newsletter) {
    ...
    if (picked.kind === 'video') {
        ...
        if (!seconds || Number(seconds) <= 0)
            seconds = await probeSeconds(buffer, 'mp4');
    ...
    if (picked.kind === 'audio') {
        let seconds = picked.msg?.seconds;
        if (!seconds || Number(seconds) <= 0)
            seconds = await probeSeconds(buffer, 'mp3');
        if (helper?.sendNewsletterMessage) {
...
function collectChannelCandidates(ctx, ownerJid) {
    ...
    for (const st of stores) {
        ...
        for (const id of Object.keys(chats)) {
            if (String(id).endsWith('@newsletter'))
                set.add(id);
        }
    }
    return [...set];
...
export async function handleListChannel(ctx) {
    ...
    let ownerJid = '';
    try {
        ownerJid = await resolveOwnerChannelJid(sock, helper);
    }
    catch { }
...
export async function handleUpChannel(ctx) {
    ...
    if (!rawQuoted) {
        await sock.sendMessage(from, { text: 'Reply pesan/media dulu lalu ketik .upch\nAtau: .upch teks (tanpa reply) untuk kirim teks ke channel.' }, { quoted: msg });
        return;
    }
...
export async function upch(sock, target, input, options = {}) {
    ...
    if (input && typeof input === 'object') {
        ...
        try {
            ...
            if (text)
                return await sock.sendMessage(channelJid, { text }, options);
            throw error;
        }
    }
