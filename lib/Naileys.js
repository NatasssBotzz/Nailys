...
async function uploadStickerPackBlob(buffer, ctx = {}, fileName = 'sticker-pack.zip') {
    ...
    const { mediaKey, encFilePath, originalFilePath, fileEncSha256, fileSha256, fileLength } = await encryptedStream(buffer, 'sticker-pack', { logger: ctx?.logger, saveOriginalFileIfRequired: false, opts: ctx?.options });
    try {
    ...
    sock.sendButton = async (jid, opts = {}) => {
        ...
        if (opts.buttons) {
            for (const btn of opts.buttons) {
                if (btn.type === 'url')
                    b.urlbtn(btn.text, btn.url);
                else if (btn.type === 'copy')
                    b.copybtn(btn.text, btn.copyText);
                else if (btn.type === 'call')
                    b.callbtn(btn.text, btn.phone);
                else
                    b.qreplybtn(btn.text, btn.id || btn.text);
            }
        }
        if (opts.contextInfo)
            b.setContextInfo(opts.contextInfo);
