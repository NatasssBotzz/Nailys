...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const relayNewsletterMessage = async (
		newsletterJid: string,
		message: proto.IMessage,
		relayOptions: MessageRelayOptions = {}
	) => {
		const jid = normalizeNewsletterJid(newsletterJid)
		const msgId = relayOptions.messageId || generateMessageIDV2(sock.user?.id)
		const additionalAttributes = { ...(relayOptions.additionalAttributes || {}), newsletter: 'true' }
		await relayMessage(jid, message, { ...relayOptions, messageId: msgId, additionalAttributes })
		return msgId
	}
	...
	const sendNewsletterMessage = async (
		...
		options: MiscMessageGenerationOptions & { additionalAttributes?: BinaryNodeAttributes } = {}
	) => {
		const jid = normalizeNewsletterJid(newsletterJid)
		const userJid = authState.creds.me!.id
		const fullMsg = await generateWAMessage(jid, content, {
		...
		await relayNewsletterMessage(jid, fullMsg.message!, {
			messageId: fullMsg.key.id!,
	...
	const sendNewsletterMedia = async (
		newsletterJid: string,
		content: AnyMessageContent,
		options: MiscMessageGenerationOptions & { additionalAttributes?: BinaryNodeAttributes } = {}
	) => sendNewsletterMessage(newsletterJid, content, options)

	const sendNewsletterText = async (
		newsletterJid: string,
		text: string,
		options: MiscMessageGenerationOptions & { contextInfo?: proto.IContextInfo; additionalAttributes?: BinaryNodeAttributes } = {}
	) => {
		return sendNewsletterMessage(
			newsletterJid,
			{ text: text || ' ', contextInfo: options.contextInfo },
			options
		)
	}

	const sendNewsletterButtons = async (
		newsletterJid: string,
		payload: { text: string; footer?: string; buttons: { id: string; text: string; type?: number }[]; contextInfo?: proto.IContextInfo },
		options: MessageRelayOptions = {}
	) => {
		const message = makeLegacyButtonsMessage(payload)
		return relayNewsletterMessage(newsletterJid, message, options)
	}

	const sendStatusText = async (
		text: string,
		statusJidList?: string[],
		options: MiscMessageGenerationOptions & { contextInfo?: proto.IContextInfo } = {}
	) => sendMessage('status@broadcast', { text: text || ' ', contextInfo: options.contextInfo }, { ...options, statusJidList, broadcast: true })

	const sendStatusMedia = async (
		content: AnyMessageContent,
		statusJidList?: string[],
		options: MiscMessageGenerationOptions = {}
	) => sendMessage('status@broadcast', content, { ...options, statusJidList, broadcast: true })
