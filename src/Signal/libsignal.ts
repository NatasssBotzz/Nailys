			...
			return parsedKeys.transaction(
				async (): Promise<{ migrated: number; skipped: number; total: number }> => {
					...
					type MigrationOp = {
						...
						lidUser: string
						deviceId: number
				fromAddr: InstanceType<typeof ProtocolAddress>
				toAddr: InstanceType<typeof ProtocolAddress>
					}
...
function signalStorage(
	...
	// Use original address directly — no PN→LID resolution for signal sessions
	// Sessions are stored under PN addresses (@s.whatsapp.net) for consistency
	...
	return {
		...
		storeSession: async (id: string, session: InstanceType<typeof SessionRecord>) => {
			const wireJid = await resolveLIDSignalAddress(id)
		...
		removePreKey: async (id: number) => { await keys.set({ 'pre-key': { [id]: null } }) },
		loadSignedPreKey: () => {
