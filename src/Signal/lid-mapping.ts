...
export class LIDMappingStore {
	...
	async storeLIDPNMappings(pairs: LIDMapping[]): Promise<void> {
		...
		// Only update in-memory cache — no database writes
		for (const { pnUser, lidUser } of validatedPairs) {
			const existingLidUser = this.mappingCache.get(`pn:${pnUser}`)
			if (existingLidUser === lidUser) {
				this.logger.debug({ pnUser, lidUser }, 'LID mapping already cached, skipping')
				continue
			}

			this.mappingCache.set(`pn:${pnUser}`, lidUser)
			this.mappingCache.set(`lid:${lidUser}`, pnUser)
			this.logger.trace({ pnUser, lidUser }, 'Stored LID-PN mapping in cache')
		}
	}
