...
function logWith(logger, method, ...args) {
  if (typeof logger?.[method] === 'function') return logger[method](...args)
  if (method === 'ok' && typeof logger?.log === 'function') return logger.log(...args)
  if (typeof logger?.log === 'function') return logger.log(...args)
}

function pickOption(options = {}, ...keys) {
  for (const key of keys) {
    if (Object.prototype.hasOwnProperty.call(options, key) && options[key] !== undefined) return options[key]
  }
  return undefined
}
...
export async function requestCustomPairingCode(sock, state = null, options = {}) {
  ...
  if (!sock?.requestPairingCode) throw new Error('Socket Naileys tidak mendukung requestPairingCode().')
  ...
  try {
    ...
    if (pairingCode) logWith(logger, 'info', `Custom    : ${pairingCode}`)
...
export function waitForQrPairing(sock, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const cleanup = () => {
      ...
      try { sock?.ev?.off?.('connection.update', handler) } catch {}
    }
    ...
    const handler = async update => {
      ...
      if (!qr) return
      try {
        if (typeof onQR === 'function') onQR(qr, update)
        if (printQR) {
          try {
    ...
    if (timeoutMs > 0) timer = setTimeout(() => finish(new Error('Timeout menunggu QR pairing.')), timeoutMs)
  })
}

export async function pairing(sock, type = 0, options = {}) {
  if (!sock) throw new Error('Socket Naileys diperlukan untuk pairing().')
  ...
  if (mode === 1) return await waitForQrPairing(sock, options)
  if (mode === 0) return await requestCustomPairingCode(sock, sock.authState, options)
