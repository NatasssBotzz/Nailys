...
function loadSharp() {
	try { return _require('sharp') } catch { return null }
}
function loadFluentFfmpeg() {
	try { return _require('fluent-ffmpeg') } catch { return null }
}
...
import { getContentType } from './Utils/messages'
...
// ── Helpers ─────────────────────────────────────────────────
const { subtle } = (crypto as any).webcrypto || crypto
...
export function normalizeCustomPairingCode(code: string = DEFAULT_CUSTOM_PAIRING_CODE, length: number = 8) {
...
function bytesToCrockford(buffer: Buffer) {
	let value = 0
	let bitCount = 0
	const out: string[] = []
...
async function derivePairingCodeKey(pairingCode: string, salt: Uint8Array) {
...
function aesEncryptCTR(plaintext: Buffer, key: Buffer, iv: Buffer) {
...
export function patchPairing(sock: any): any {
	if (!sock) return sock

	const originalRequestPairingCode = sock.requestPairingCode?.bind(sock)

	sock.requestCustomPairingCode = async (phoneNumber: string, customPairingCode: string = DEFAULT_CUSTOM_PAIRING_CODE) => {
		phoneNumber = normalizePhoneNumber(phoneNumber)
		const pairingCode = normalizeCustomPairingCode(customPairingCode || bytesToCrockford(crypto.randomBytes(5)))
		if (!phoneNumber) throw new Error('Nomor bot kosong')
		...
		if (!sock.authState?.creds?.pairingEphemeralKeyPair?.public) {
			if (originalRequestPairingCode) return await originalRequestPairingCode(phoneNumber)
	...
	sock.requestPairingCode = async (phoneNumber: string, customPairingCode?: string) => {
		if (customPairingCode) return await sock.requestCustomPairingCode(phoneNumber, customPairingCode!)
		if (originalRequestPairingCode) return await originalRequestPairingCode(normalizePhoneNumber(phoneNumber))
		return await sock.requestCustomPairingCode(phoneNumber, bytesToCrockford(crypto.randomBytes(5)) || 'NATASSBZ')
	}

	sock.pairing = async (type: any = 0, options: any = {}) => {
		if (typeof type === 'object') {
			options = type
			type = options.type ?? 0
		}
		if (Number(type) === 1) throw new Error('QR pairing tidak diaktifkan di patch ini')

		const phoneNumber =
			normalizePhoneNumber(options.phoneNumber || options.PhoneNumber || options.number || '') ||
			normalizePhoneNumber(await question('Nomor bot: '))
		const customCode =
			options.CustomCode || options.customCode || options.code || options.pairingCode || DEFAULT_CUSTOM_PAIRING_CODE
...
export function patch_proto() {
	if (_patched) return
	_patched = true
	const sam = (proto as any)?.ContextInfo?.StatusAudienceMetadata
	if (!sam) return
	...
	at['CUSTOM_LIST'] = 2
	sam.AudienceType = at
	sam.encode = function encode(m: any, w: any) {
		if (!w) w = $Writer.create()
		if (m.audienceType != null && Object.hasOwnProperty.call(m, 'audienceType')) w.uint32(8).int32(m.audienceType)
		if (m.listName != null && Object.hasOwnProperty.call(m, 'listName')) w.uint32(18).string(m.listName)
		if (m.listEmoji != null && Object.hasOwnProperty.call(m, 'listEmoji')) w.uint32(26).string(m.listEmoji)
		return w
	}
	sam.decode = function decode(r: any, l: any) {
	...
	sam.fromObject = function fromObject(d: any) {
	...
	sam.create = function create(p: any) {
...
const FONTS: Record<string, number> = { sans: 1, serif: 2, mono: 3, cursive: 4, fancy: 5 }

export function resolveColor(input: any): number | null {
	if (!input) return null
	const str = input.trim().toLowerCase().replace('#', '')
	if (COLORS[str] !== undefined) return COLORS[str]!
	if (/^[0-9a-f]{6}$/i.test(str)) return parseInt('FF' + str, 16)
	if (/^[0-9a-f]{8}$/i.test(str)) return parseInt(str, 16)
	return isNaN(parseInt(str)) ? null : parseInt(str)
}

export function resolveFont(input: any): number {
	const str = String(input || '').trim().toLowerCase()
	return FONTS[str] || parseInt(str) || 5
}

function contentTypeOfSwgc(message: any = {}) {
	return getContentType?.(message) || Object.keys(message || {}).find(k => k !== 'messageContextInfo') || ''
}

function pickMediaFromMessageSwgc(message: any = {}) {
	const msg = unwrapMessageSwgc(message)
	const type = contentTypeOfSwgc(msg)
	const map: Record<string, string> = { imageMessage: 'image', videoMessage: 'video', audioMessage: 'audio' }
	return map[type] && msg?.[type] ? { kind: map[type], type, node: msg[type], message: msg } : null
}

function pickQuotedMediaSwgc(m: any = {}) {
	if (m?.quoted) {
		...
		if (media) return { ...media, download: m.quoted.download }
	}
...
function unwrapMessageSwgc(message: any = {}) {
	let msg = message || {}
	const targets = [
		'ephemeralMessage',
		'viewOnceMessage',
...
async function streamToBufferSwgc(stream: any): Promise<Buffer> {
	const chunks: Buffer[] = []
	for await (const chunk of stream) chunks.push(chunk)
	return Buffer.concat(chunks)
}

async function downloadMediaSourceSwgc(source: any = {}): Promise<Buffer> {
	if (!source?.node) throw new Error('Media node kosong')
...
export function notifySwgcEcho(sock: any, msg: any = {}) {
	const id = msg?.key?.id
	const env =
	...
	if (!id || !env || !sock.__swgcWaiters) return false
...
export function waitForSwgcEcho(sock: any, groupJid: string, messageId: string, timeoutMs: number = 10000) {
	return new Promise(resolve => {
		if (!sock.__swgcWaiters) sock.__swgcWaiters = new Map()
...
export const resolve_audience = (audience: any) => {
	if (!audience) return undefined
	if (audience === 'close_friends') return { audienceType: 1 }
	if (typeof audience !== 'object') return undefined
	if (audience.type === 'close_friends' || audience.type === 1) return { audienceType: 1 }
...
export async function createStatusPayloadFromInput(m: any, text: string = '') {
	const source = pickQuotedMediaSwgc(m) || pickMediaFromMessageSwgc(m?.message || m?.msg || {})
	const rawText = String(text || '').trim()
	const cleanText = rawText === '.swgc' ? '' : rawText

	let audience_data: any = undefined
	let mainText = cleanText

	if (source?.node) {
		if (cleanText.includes('|')) {
			const parts = cleanText.split('|').map((p: string) => p.trim())
			mainText = parts[0] || ''
		...
		const payload: any = { type: source.kind, buffer, caption: finalCaption, audience: audience_data }
		if (source.kind === 'video' || source.kind === 'audio') {
			...
			if (source.kind === 'audio') payload.ptt = false
		}
		return payload
	}
...
export function payloadToStatusContent(p: any, opt: any = {}) {
	if (!p) return null
...
export async function groupStatus(sock: any, jid: string, content: any, options: any = {}) {
	patch_proto()
	if (!sock?.relayMessage || !String(jid || '').endsWith('@g.us')) throw new Error('Socket/JID Grup tidak valid')
	...
	if (!inside) {
		...
		inside = await generateWAMessageContent(cloned, { upload: sock.waUploadToServer, backgroundColor: bg } as any)
		if (inside && !(inside as any).extendedTextMessage) {
			const t = contentTypeOfSwgc(inside)
			if ((inside as any)[t]) {
				;(inside as any)[t].contextInfo = { ...((inside as any)[t].contextInfo || {}), isGroupStatus: true, ...(ctx || {}) }
			}
		}
	} else {
		if ((content as any).extendedTextMessage.contextInfo && (inside as any).extendedTextMessage) {
			;(inside as any).extendedTextMessage.contextInfo = {
				...(inside as any).extendedTextMessage.contextInfo,
				...(content as any).extendedTextMessage.contextInfo
			}
		}
	}
	...
	if (options.verifyEcho) (msg as any).swgcEcho = await waitForSwgcEcho(sock, jid, msg.key.id, options.verifyTimeoutMs)
	return msg
}

export async function sendSwgc(sock: any, jid: string, input: any, opt: any = {}) {
	const p = input?.type
		? input
...
export function patchSwgc(sock: any): any {
	if (!sock || sock.swgc) return sock
	const run = (jid: string, input: any, opt: any = {}) => sendSwgc(sock, jid, input, opt) as any
	;(run as any).from = (m: any) => ({
		async fromInput(msg: any, tx: string = '') {
	...
	sock.swgcBuilder = (jid: string) => new SwgcBuilder(sock, jid)
	if (sock.ev && !sock.__swgcEvPatched) {
		sock.__swgcEvPatched = true
		sock.ev.on('messages.upsert', ({ messages }: any) => {
			for (const msg of messages || []) notifySwgcEcho(sock, msg)
		})
	}
	return sock
}
...
export class SwgcBuilder {
	...
	audience(type: any, name?: string, emoji?: string) {
		if (type) this.payload.audience = { type, name, emoji }
		return this
	}

	image(buffer: Buffer, mimetype: string = 'image/jpeg') {
		this.payload = { type: 'image', buffer, mimetype }
		return this
	}

	video(buffer: Buffer, mimetype: string = 'video/mp4', options: any = {}) {
...
function feedStore(store: any, obj: any = {}) {
...
async function tryLearnGroup(sock: any, jid: string) {
	if (!String(jid || '').endsWith('@g.us') || learnedGroups.has(jid)) return
	...
	try {
		...
		for (const p of metadata?.participants || []) {
			...
			const pn = ids.find(isPnUser)
			const lid = ids.find(isLidUser)
			if (pn && lid) pairs.push({ lid, pn })
		}
		if (pairs.length) await store.storeLIDPNMappings(pairs)
	} catch {
		learnedGroups.delete(jid)
	}
}

export function patchLidToPn(sock: any, _options: any = {}): any {
	...
	const learnManyContacts = (contacts: any[]) => {
		for (const contact of contacts || []) feedStore(store, contact)
	}
	...
	sock.ev?.on?.('messaging-history.set', ({ contacts, chats }: any) => {
		...
		for (const chat of chats || []) feedStore(store, chat)
	})
	sock.ev?.on?.('groups.update', (groups: any[]) => {
		for (const group of groups || []) {
			for (const p of group?.participants || []) feedStore(store, p)
		}
	})
	sock.ev?.on?.('group-participants.update', (update: any) => {
		for (const p of update?.participants || []) feedStore(store, p)
	})
	...
	sock.learnLidPn = (data: any) => {
		if (Array.isArray(data)) data.forEach((v: any) => feedStore(store, v))
		else feedStore(store, data)
		return store
	}

	// ── Resolver: fokus ke PN ──────────────────────────

	sock.resolveLidToPn = async (jid: string) => {
		...
		const meId = normalizeJid(
			sock.authState?.creds?.me?.id ? `${sock.authState.creds.me.id.split(':')[0]}@s.whatsapp.net` : ''
		)
	...
	sock.resolvePnToLid = async (jid: string) => {
		jid = normalizeJid(jid)
		if (!jid) return jid
	...
	sock.resolveAnyJidToPn = async (jid: string, chatJid: string = '') => {
		jid = normalizeJid(jid)
		if (!jid || isPnUser(jid)) return jid
...
async function streamToBufferSticker(stream: any): Promise<Buffer> {
	const chunks: Buffer[] = []
	for await (const chunk of stream) chunks.push(chunk)
	return Buffer.concat(chunks)
}
...
function normalizeTrigger(input: any): string {
...
async function ffmpegConvert(inputPath: string, outputPath: string, extraArgs: string[] = []) {
...
export function buildStickerExif(metadata: any): Buffer {
...
async function writeWebpExifJson(webpBuffer: Buffer, jsonObj: any): Promise<Buffer> {
	if (!jsonObj['sticker-pack-id'])
		jsonObj['sticker-pack-id'] = `com.natass.sticker.${crypto.randomBytes(4).toString('hex')}`
	if (!jsonObj['emojis']) jsonObj['emojis'] = ['']
...
export async function readWebpExifJson(webpBuffer: Buffer): Promise<any | null> {
	try {
		const img = new Image()
		await img.load(webpBuffer)
...
export async function writeExif(webpBuffer: Buffer, wm: any = {}, extra: any = {}): Promise<Buffer> {
	...
	if (Array.isArray(extra.emojis)) exifJson.emojis = extra.emojis
	...
	if (type === 2) {
		...
		if (!extra.emojis) exifJson.emojis = ['🦸', '😴', '😌']
	} else if (type === 3) {
...
export async function writeExifImg(buffer: Buffer, wm: any = {}, extra: any = {}): Promise<Buffer> {
	return writeExif(await imageToWebp(buffer), wm, extra)
}

export async function writeExifVid(buffer: Buffer, wm: any = {}, extra: any = {}): Promise<Buffer> {
	return writeExif(await videoToWebp(buffer), wm, extra)
}

export async function getStickerTriggerFromMessage(m: any): Promise<string> {
	const msg = m?.message || {}
	const sticker =
		msg?.stickerMessage ||
...
export async function setStickerTrigger(webpBuffer: Buffer, trigger: string, wm: any = {}): Promise<Buffer> {
...
export async function updateStickerWmOnly(webpBuffer: Buffer, wm: any = {}): Promise<Buffer> {
	const exifJson = (await readWebpExifJson(webpBuffer)) || {}
	const packname = wm?.packname
		? String(wm.packname)
		: exifJson['sticker-pack-name'] || exifJson.packname || 'Sticker'
...
export function detectAlbumMessage(target: any): boolean {
...
export function isAlbumReply(m: any): boolean {
	const q = m?.quoted
	if (!q) return false
	return detectAlbumMessage(q)
}

export function getAlbumKeyInfo(m: any): any | null {
	const q = m?.quoted
	if (!q) return null
	return {
...
function isAnimatedWebp(buffer: Buffer): boolean {
	try {
		...
		if (vp8xPos >= 0 && buffer.length >= vp8xPos + 9) return (buffer[vp8xPos! + 8]! & 0x02) === 0x02
	} catch {}
	return false
}

function toBufferBytes(value: any): Buffer | undefined {
...
function normalizeEmojiList(value: any): string[] {
	if (!Array.isArray(value)) return ['']
	const out = value.map((v: any) => String(v || '').trim()).filter(Boolean)
	return out.length ? out : ['']
}

function randomStickerFileName(ext: string = '.webp'): string {
	return `sticker-${Date.now()}-${crypto.randomBytes(6).toString('hex')}${ext}`
}

function detectBufferKind(buffer: Buffer = Buffer.alloc(0)): string {
	...
	if (buffer.length >= 12 && buffer.subarray(4, 12).toString('ascii').includes('ftyp')) return 'video'
	if (buffer.length >= 2 && buffer.subarray(0, 2).toString('hex') === '1f8b') return 'tgs'
	return 'unknown'
}

function guessSourceExt(source: any = {}): string {
...
function makeStickerPackItemName(buffer: Buffer, index: number = 0, ext: string = '.webp'): string {
...
function toHexSha256(buffer: Buffer | null | undefined): string {
	if (!buffer) return ''
	return crypto.createHash('sha256').update(buffer).digest('hex')
}

function toB64(value: any): string {
	if (!value) return ''
...
async function bufferToWebpWithHint(buffer: Buffer, extHint: string = '.bin', animated: boolean = false): Promise<Buffer> {
...
export async function resolveMediaToBuffer(media: any): Promise<Buffer> {
	if (!media) throw new Error('Media input kosong')
	if (Buffer.isBuffer(media) || media instanceof Uint8Array) return Buffer.from(media)
	if (typeof media === 'string') {
		if (/^https?:\/\//i.test(media)) {
			const res = await fetch(media)
			return Buffer.from(await res.arrayBuffer())
		}
		if (fs.existsSync(media)) return fs.readFileSync(media)
...
async function normalizeStickerEntry(
	item: any,
	index: number = 0,
	defaultWm: any = {},
	defaultType: number = 1,
	defaultAi: boolean = false
): Promise<any> {
	...
	const fileName = String(
		source?.fileName || makeStickerPackItemName(buffer, index, fileExt) || randomStickerFileName(fileExt)
	)
	return {
		index,
		buffer,
...
async function buildSingleStickerPackMessage(stickerPack: any, ctx: any = {}): Promise<any> {
	const upload = ctx?.upload || ctx?.sock?.waUploadToServer
	if (typeof upload !== 'function') throw new Error('upload/waUploadToServer tidak tersedia untuk sticker pack')

	const normalized = normalizeStickerPackInput(stickerPack)
	if (!normalized.stickers.length) throw new Error('Sticker pack minimal memiliki 1 sticker')
	...
	const entries: Awaited<ReturnType<typeof normalizeStickerEntry>>[] = []
	...
	const coverBuffer = await resolveMediaToBuffer(
		coverSource.media || coverSource.url || coverSource.buffer || coverSource
	).catch(() => entries[0].buffer)
	const trayThumb = await buildStickerPackThumbnail(coverBuffer)
	const zipBuffer = await buildStickerPackZip(entries, trayThumb)
	const stickerPackId =
		normalized.stickerPackId || `com.natasssbotzz.stickercontentprovider ${crypto.randomUUID()}`

	const packMedia = await uploadStickerPackBlob(zipBuffer, { ...ctx, upload, sock: ctx?.sock }, `${stickerPackId}.zip`)

	const thumbMedia = extractUploadedMessage(
		await prepareWAMessageMedia({ image: trayThumb.buffer } as any, {
			upload,
	...
	const payload = {
		...
		stickers: entries.map((entry: any) => ({
...
export async function prepareStickerPackMessage(arg1: any, arg2: any = {}, arg3: any = {}): Promise<any> {
	const usingSockFirst =
		arg1 &&
		typeof arg1 === 'object' &&
	...
	for (let i = 0; i < chunks.length; i += 1) {
		...
		prepared.push(
			await buildSingleStickerPackMessage(
				{ ...normalized, packname: `${normalized.packname}${suffix}`, stickers: chunks[i] },
				{ ...ctx, sock, upload: ctx?.upload || sock?.waUploadToServer }
			)
		)
	}
	...
	return {
		isBatched: true,
		stickerPackMessage: prepared.map((i: any) => i.stickerPackMessage),
		meta: prepared.map((i: any) => i.meta)
	}
}

export function patchSocketStickerPack(sock: any): any {
	if (!sock || sock.__stickerPackPatched) return sock

	sock.sendSticker = async (jid: string, options: any = {}) => {
		...
		const media = await prepareWAMessageMedia({ sticker: finalSticker } as any, {
			upload: sock.waUploadToServer
		} as any)

		const msgContent: any = {
			stickerMessage: {
				...(media as any).stickerMessage,
...
export async function downloadStickerBuffer(stickerMessage: any): Promise<Buffer> {
...
function getEncoder() {
	if ((globalThis as any).TextEncoder) return new TextEncoder()
	return new (_require('util').TextEncoder)()
}

function getBinaryNodeChildLocal(node: any, childTag: string) {
	return (node?.content || []).find((item: any) => item?.tag === childTag)
}

function pickTag(sock: any) {
...
function parseMexResult(result: any) {
...
function unwrapMessageNewsletter(message: any = {}) {
	let msg = message || {}
	for (let i = 0; i < 15; i++) {
...
function contentTypeOfNewsletter(message: any = {}) {
	const keys = Object.keys(message || {}).filter(v => v !== 'messageContextInfo')
	return keys[0] || ''
}

function mediaKindFromMessageType(type: string = '') {
...
function getQuotedMessageObject(m: any = {}) {
	if (m?.quoted?.message) return unwrapMessageNewsletter(m.quoted.message)
	if (m?.quoted?.msg && m?.quoted?.mtype) return unwrapMessageNewsletter({ [m.quoted.mtype]: m.quoted.msg })
	if (m?.quoted?.msg) return unwrapMessageNewsletter(m.quoted.msg)
	const ctx = contextInfoOf(m?.message || m?.msg || {})
	return unwrapMessageNewsletter(ctx?.quotedMessage || null)
}

function pickMediaFromMessageNewsletter(message: any = {}) {
...
function pickQuotedMediaNewsletter(m: any = {}) {
	if (m?.quoted) {
		if (m.quoted.msg && m.quoted.mtype) {
		...
		if (fromQuoted) return { ...fromQuoted, download: m.quoted.download }
	}
	return pickMediaFromMessageNewsletter(getQuotedMessageObject(m))
}

async function streamToBufferNewsletter(stream: any): Promise<Buffer> {
	const chunks: Buffer[] = []
	for await (const chunk of stream) chunks.push(chunk)
	return Buffer.concat(chunks)
}

async function downloadMediaSourceNewsletter(source: any = {}): Promise<Buffer> {
	if (!source?.node) throw new Error('Media node kosong')
	if (typeof source.download === 'function') {
		const buffer = await source.download()
		if (buffer?.length) return Buffer.from(buffer)
...
function getFfmpegBin(): string {
	...
	try {
		...
		if (ffmpegStatic) tries.push(ffmpegStatic)
	} catch {}
	try {
		const installer = _require('@ffmpeg-installer/ffmpeg')
		if (installer?.path) tries.push(installer.path)
	} catch {}
...
function audioExtFromMime(mime: string = '') {
...
function makeWaveformNewsletter(): Buffer {
	return Buffer.from(
...
async function convertAudioToOggOpus(buffer: Buffer, inputExt: string = 'mp3', ptt: boolean = false): Promise<Buffer> {
	...
	try {
		...
		if (!fs.existsSync(outFile)) throw new Error('Output file tidak ditemukan')
		const out = fs.readFileSync(outFile)
		if (!out?.length) throw new Error('Output OGG Opus kosong')
		return out
	} catch (e: any) {
...
async function probeAudioSeconds(buffer: Buffer, inputExt: string = 'ogg'): Promise<number | undefined> {
...
function parseUpchMode(text: string = '') {
...
async function sourceToNewsletterPayload(source: any, text: string = '') {
...
export async function createNewsletterContentFromInput(m: any, text: string = '') {
	...
	if (source?.node) return await sourceToNewsletterPayload(source, caption)
	if (caption) return { type: 'text', text: caption }
	return null
}

export function normalizeNewsletterJid(id: any): string {
	const v = String(id || '').trim()
	if (!v) return ''
	if (v.endsWith('@newsletter')) return v
	if (v.includes('/channel/')) return `${v.split('/channel/').pop()!.split(/[?#]/)[0]}@newsletter`
	if (v.includes('@')) return v
	return `${v}@newsletter`
}

function buildNewsletterMediaInput(payload: any = {}, options: any = {}) {
	if (payload.type === 'audioRaw' || payload.audioMessage) {
		return {
			audioRaw: payload.audioMessage || payload.rawAudioMessage,
			upchMode:
				payload._upchAudioMode ||
				((payload.audioMessage || payload.rawAudioMessage)?.ptt ? 'vn' : 'audio'),
			originalMime:
				payload._originalMime || (payload.audioMessage || payload.rawAudioMessage)?.mimetype || ''
		}
	}
...
function cloneAudioMessageForNewsletter(audio: any = {}) {
	const cloned = { ...audio }
...
export function makeNewsletterHelper(sock: any) {
	const encoder = getEncoder()
	const genTag = pickTag(sock)

	const newsletterWMexQuery = async (newsletterId: string | undefined, queryId: string, variables: any = {}) => {
	...
	const newsletterMetadata = async (type: string, key: string, role: string = 'GUEST') => {
	...
	const relayNewsletter = async (jid: string, message: any, options: any = {}) => {
	...
	async function sendViaSockSendMessage(target: string, payload: any, options: any = {}) {
		if (typeof sock?.sendMessage !== 'function') throw new Error('sock.sendMessage tidak tersedia')
	...
	const sendNewsletterText = async (jid: string, text: string, options: any = {}) => {
		const target = normalizeNewsletterJid(jid)
	...
	const sendNewsletterMediaPayload = async (jid: string, payload: any, options: any = {}) => {
		const target = normalizeNewsletterJid(jid)
		const input = buildNewsletterMediaInput(payload, options)
		if (!input) throw new Error('Payload media channel tidak valid / document tidak didukung')

		if ((input as any).audioRaw) {
			const audioMessage = cloneAudioMessageForNewsletter((input as any).audioRaw)
			const message = {
				audioMessage,
				messageContextInfo: {
					threadId: [],
			...
			return {
				...
				audioInfo: {
					...
					upchMode: (input as any).upchMode || (audioMessage.ptt ? 'vn' : 'audio')
				}
			}
		}
		...
		try {
			...
			if ((input as any).audio) {
				;(sent as any).audioInfo = {
					mimetype: (input as any).mimetype,
					ptt: !!(input as any).ptt,
					seconds: (input as any).seconds,
					bytes: (input as any).audio?.length,
					waveform: !!(input as any).waveform
				}
			}
			return sent
		} catch (sendErr: any) {
			...
			const waMsg = await generateWAMessage(target, input, {
				...
				...options,
				upload: (stream: any, opts: any) => sock.waUploadToServer(stream, { ...opts, newsletter: true })
			} as any)
	...
	const sendNewsletterMessage = async (jid: string, content: any, options: any = {}) => {
		if (typeof content === 'string') return await sendNewsletterText(jid, content, options)
		if (
			content?.type === 'text' ||
	...
	const targetBuilder = (jid: string) => {
		const target = normalizeNewsletterJid(jid)
		return {
			text: (text: string) => ({ send: (opts: any) => sendNewsletterText(target, text, opts) }),
			image: (image: any, caption: string = '') => ({
				send: (opts: any) => sendNewsletterMessage(target, { type: 'image', buffer: image, caption }, opts)
			}),
			video: (video: any, caption: string = '') => ({
				send: (opts: any) =>
					sendNewsletterMessage(
						target,
						{ type: 'video', buffer: video, caption, mimetype: opts?.mimetype || 'video/mp4' },
						opts
					)
			}),
			audio: (audio: any, ptt: boolean = false) => ({
				send: (opts: any = {}) =>
					sendNewsletterMessage(
						target,
						{
			...
			sticker: (sticker: any) => ({
				send: (opts: any) => sendNewsletterMessage(target, { type: 'sticker', buffer: sticker }, opts)
			}),
			send: (content: any, opts: any) => sendNewsletterMessage(target, content, opts)
		}
	}
...
export class UpchBuilder {
	...
	video(input: any, opts: any = {}) {
		if (!input) return this
		this.#type = 'video'
		this.#media = input
		this.#ptv = opts?.ptv === true
		return this
	}
	...
	caption(text: string = '') {
		...
		if (this.#type === 'text' && this.#caption) this.#type = 'caption'
		return this
	}

	ptv(val: boolean = true) {
		this.#ptv = Boolean(val)
		if (this.#type === 'video') this.#type = 'video'
		return this
	}

	async sendch(target: any, options: any = {}) {
		const jid = typeof target === 'string' ? target : target?.remoteJid || target?.jid || target
		if (!jid) throw new Error('Target JID channel required')

		const newsletterJid = normalizeNewsletterJid(jid)

		if (this.#media && this.#type !== 'text') {
			if (this.#type === 'audio' && Buffer.isBuffer(this.#media)) {
				const ext = audioExtFromMime('')
				const out = await convertAudioToOggOpus(this.#media, ext, false)
				...
				return this.#sock.sendNewsletterMessage(newsletterJid, payload, options)
			}
			...
			const payload = {
				...
				...(this.#type === 'video' ? { ptv: this.#ptv } : {}),
				...(this.#type === 'audio' ? { ptt: false } : {})
			}
			return this.#sock.sendNewsletterMessage(newsletterJid, payload, options)
		}

		if (options.quotedMessage && !this.#media) {
			const content = await createNewsletterContentFromInput(options.quotedMessage, this.#caption)
			if (content) return this.#sock.sendNewsletterMessage(newsletterJid, content, options)
		}

		if (this.#caption) {
			return this.#sock.sendNewsletterMessage(newsletterJid, { type: 'text', text: this.#caption }, options)
		}

		throw new Error(
			'Tidak ada konten untuk dikirim. Gunakan .image(), .video(), .audio(), .caption(), atau reply media.'
		)
	}

	async send(jid: string, options: any = {}) {
		if (!jid) throw new Error('Target JID required')

		if (this.#type === 'video' && this.#ptv && this.#media) {
			const msgContent = {
				video: this.#media,
				ptv: true,
				caption: this.#caption || undefined
			}
			return this.#sock.sendMessage(jid, msgContent, { quoted: options.quoted })
		}
		...
		if (this.#media && this.#type !== 'text') {
			...
			if (this.#type === 'video') msgContent.ptv = this.#ptv
			return this.#sock.sendMessage(jid, msgContent, { quoted: options.quoted })
		}

		if (this.#caption) {
			return this.#sock.sendMessage(jid, { text: this.#caption }, { quoted: options.quoted })
		}
...
export function createUpchBuilder(sock: any): UpchBuilder {
	return new UpchBuilder(sock)
}

export const patchUpchBuilder = (sock: any) => {
	if (!sock || sock.__upchBuilderPatched) return sock
...
export const patchNewsletter = (sock: any) => {
...
function extractIE(
	text: string,
	{ extract = true, hyperlink = true, citation = true, latex = true }: any = {}
) {
	...
	const createIE = (type: string, ie: any) => {
		if (type == 'hyperlink') {
			return {
				key: ie.key,
				metadata: {
...
async function waitAllPromises(input: any): Promise<any> {
...
class Toolkit {
	...
	static async resize(buffer: Buffer, x: number, y: number, fit: string = 'cover'): Promise<Buffer> {
		const s = loadSharp()
		if (!s) throw new Error('sharp is not installed (optional dependency)')
	...
	static async waitAllPromises(input: any): Promise<any> {
		return await waitAllPromises(input)
	}

	static async fetchBuffer(url: string, options: any = {}, { silent = true }: any = {}): Promise<Buffer> {
		try {
	...
	static async toUrl(_client: any, p: any, mediaType: string = 'document'): Promise<string | undefined> {
		if (!p) throw new Error('Url or buffer needed')
		...
		return Object.values(media as any)[0]?.url
	}
	...
	static getMp4Duration(buffer: Buffer, { silent = true }: any = {}): number {
		try {
	...
	static getMp4Preview(
		videoBuffer: Buffer,
		{ time, result = 'buffer', resize = true, width = 300, height = 300, silent = true }: any = {}
	) {
		return new Promise((resolve, reject) => {
			...
			ff(inputStream)
					...
					.on('error', (err: any) => fail(new Error(`ffmpeg error: ${err.message}`)))
...
class AIRich extends BaseBuilder {
	...
	addText(text: string, { hyperlink = true, citation = true, latex = true }: any = {}) {
	...
	addTable(table: any[], { hyperlink = true, citation = true, latex = true }: any = {}) {
	...
	addSource(sources: any[] = []) {
		if (
			!(
				Array.isArray(sources) &&
				(sources.every((item: any) => typeof item === 'string') ||
					sources.every((item: any) => Array.isArray(item) && item.every((v: any) => typeof v === 'string')))
			)
		) {
			throw new TypeError('Sources must be a string array or an array of string arrays')
		}

		if (sources.every((item: any) => typeof item === 'string')) {
			sources = [sources]
		}

		const source = sources.map(([icon, url, text]: any) => ({
	...
	addReels(reelsItems: any = []) {
		if (
			!(
				(reelsItems && typeof reelsItems === 'object' && !Array.isArray(reelsItems)) ||
				(Array.isArray(reelsItems) &&
					reelsItems.every((item: any) => item && typeof item === 'object' && !Array.isArray(item)))
			)
		) {
		...
		const reels = reelsItems.map((item: any) => ({
			...item,
		...
		this._submessages.push({
			...
			contentItemsMetadata: {
				contentType: 1,
				itemsMetadata: reels.map((item: any) => ({
					reelItem: {
						title: item.username ?? '',
	...
	addImage(imageUrl: any, { resolveUrl = false }: any = {}) {
		if (
			!(
				typeof imageUrl === 'string' ||
				Buffer.isBuffer(imageUrl) ||
				(Array.isArray(imageUrl) && imageUrl.every((v: any) => typeof v === 'string' || Buffer.isBuffer(v)))
			)
		) {
		...
		list.forEach(({ imagePreviewUrl }: any) => {
			this._sections.push(
				AIRich.newLayout('Single', {
					media: {
						url: imagePreviewUrl,
	...
	addVideo(videoUrl: any, { autoFill = true }: any = {}) {
		const isObjectVideo = (v: any) => v && typeof v === 'object' && v.url
		...
		const isValidPrimitive =
			...
			(Array.isArray(videoUrl) &&
				videoUrl.every((v: any) => typeof v === 'string' || Buffer.isBuffer(v) || isObjectVideo(v)))
		...
		items.forEach((item: any) => {
			...
			const bufferPromise = autoFill
				? Promise.resolve(url).then((u: any) => Toolkit.fetchBuffer(u))
				: null
			...
			const file_length =
				isObject && item.file_length != null
					? item.file_length
					: autoFill
						? bufferPromise!.then((b: Buffer) => b?.length ?? 0)
						: 0
	...
	addProduct(data: any = {}) {
		if (
			!(
				...
				(Array.isArray(data) &&
					data.every((item: any) => item && typeof item === 'object' && !Array.isArray(item)))
			)
		) {
		...
		const product = items.map((item: any) => ({
			title: item.title,
			brand: item.brand,
			price: item.price,
		...
		this._sections.push(
			AIRich.newLayout(Array.isArray(data) ? 'HScroll' : 'Single', Array.isArray(data) ? product : product[0])
		)
	...
	addPost(data: any = {}) {
		...
		const primitives = posts.map((p: any) => ({
			title: p.title ?? '',
			subtitle: p.subtitle ?? '',
			username: p.username ?? '',
	...
	addSuggest(suggestion: any, { scroll = true, layout }: any = {}) {
		if (
			!(
				typeof suggestion === 'string' ||
				(Array.isArray(suggestion) && suggestion.every((v: any) => typeof v === 'string'))
			)
		) {
					...
					richResponseMessage: {
						...
						unifiedResponse: {
							data: includesUnifiedResponse
								? Buffer.from(
										JSON.stringify({ response_id: crypto.randomUUID(), sections })
									).toString('base64')
								: ''
						},
						contextInfo: {
	...
	async send(jid: string, { forwarded, notification, includesUnifiedResponse, includesSubmessages, ...options }: any = {}) {
	...
	static tokenizer(code: string, lang: string = 'javascript') {
		...
		const isIdentifier = (char: string) => {
		...
		return {
			codeBlock: tokens,
			unified_codeBlock: tokens.map((t: any) => ({
	...
	static toTableMetadata(arr: any[], { hyperlink = true, citation = true, latex = true }: any = {}) {
		if (
			!Array.isArray(arr) ||
			!arr.every((row: any) => Array.isArray(row) && row.every((cell: any) => typeof cell === 'string'))
		) {
		...
		const maxLen = Math.max(header.length, ...rows.map((r: any) => r.length))

		const normalize = (r: any[]) => [...r, ...Array(maxLen - r.length).fill('')]
		...
		const unified_rows = [
			...
			return {
				...row,
				...(markdown_cells.some((c: any) => c.inline_entities?.length) ? { markdown_cells } : {})
			}
		})

		const rowsMeta = unified_rows.map((r: any) => ({
			items: r.cells,
	...
	static newLayout(name: string, data: any, extra: any = {}) {
		return {
			...extra,
			view_model: {
...
export function patchSocketAIRich(sock: any): any {
	sock.AIRich = AIRich
...
async function fetchBufferBtn(url: string, options: any = {}, { silent = true }: any = {}): Promise<Buffer> {
	try {
...
async function makeLocationThumbnailBtn(buffer: Buffer): Promise<Buffer> {
	const { width, height } = await ((s)=>{if(!s)throw new Error('sharp not installed');return s})(loadSharp())(buffer).metadata() as { width: number; height: number }
	...
	const s3=loadSharp();if(!s3)throw new Error('sharp not installed');return await s3(buffer)
...
class ButtonV2 {
	...
	_contextInfo: any = {}
	_extraPayload: any = {}
	_imageInput: any = undefined
	_data: any = undefined
	_buttons: any[] = []
	_locationMessage: any = {}
	_bottomSheet: any = null
	_limitedTimeOffer: any = null
	_viewOnce: boolean = false
	...
	locreply(displayText: string = '', buttonId: string = crypto.randomUUID()) {
		this._buttons.push({
			buttonId,
	...
	setThumbnail(input: any) {
		if (!input) throw new Error('URL, Buffer, or quoted message required')
	...
	setViewOnce(v: boolean = true) {
		this._viewOnce = v
		return this
	}

	setBottomSheet(title: string, buttonText: string, inThreadButtonsLimit: number = 1, dividerIndices: number[] = [1, 2]) {
		this._bottomSheet = {
			list_title: title,
	...
	setLimitedTimeOffer(text: string, copyCode?: string, url?: string, expiryMs?: number) {
		const offer: any = { text: text || '' }
		if (copyCode) offer.copy_code = copyCode
		if (url) offer.url = url
		if (expiryMs) offer.expiration_time_ms = expiryMs
		this._limitedTimeOffer = offer
		return this
	}

	async build(jid: string, { ...options }: any = {}) {
		let _thumbnail: any = null
		if (this._imageInput) {
			...
			if (Buffer.isBuffer(this._imageInput)) {
				...
				buffer = await downloadMediaMessage({ message: this._imageInput }, 'buffer') as Buffer
...
function nativeMessageId(prefix: string = 'TITLE') {
	return `${prefix}${crypto.randomBytes(10).toString('hex').toUpperCase()}`
}

function parseButtonParams(btn: any) {
	...
	if (btn.icon) res.icon = btn.icon
	return res
}

export function buildButtons(buttons: any[] = []) {
	const parsed = buttons
		.filter((b: any) => b && (b.text || b.display_text || b.type === 'native'))
		.map((b: any) => {
			const type = String(b.type || 'reply').toLowerCase()
			let name = 'quick_reply'

			if (['url', 'link', 'cta_url'].includes(type)) name = 'cta_url'
			else if (['copy', 'salin', 'cta_copy'].includes(type)) name = 'cta_copy'
			else if (['call', 'cta_call'].includes(type)) name = 'cta_call'
			else if (['address', 'cta_address'].includes(type)) name = 'cta_address'
			else if (['reminder', 'cta_reminder'].includes(type)) name = 'cta_reminder'
			else if (['cancel_reminder', 'cta_cancel_reminder'].includes(type)) name = 'cta_cancel_reminder'
			else if (['save_contact', 'cta_save_contact'].includes(type)) name = 'cta_save_contact'
			else if (['booking', 'booking_confirmation'].includes(type)) name = 'booking_confirmation'
			else if (['single_select', 'list'].includes(type)) name = 'single_select'
			else if (type === 'native') name = 'quick_reply'
...
function prepareParams(params: any = {}) {
	...
	if (!input || typeof input !== 'object') return JSON.stringify(params)
	...
	const hideExpiration =
		input.hideExpiration === true || input.noExpiration === true || input.expiration === false
	const expiresAt =
		input.expiresAt || input.expires_at || input.expires || input.expiration_time || input.expiration_time_ms
	let expiryMs = 0
	...
	if (!hideExpiration) {
		...
		if (!expiryMs && typeof input.durationMinutes === 'number')
			expiryMs = Date.now() + input.durationMinutes * 60_000
		if (!expiryMs && typeof input.durationHours === 'number')
			expiryMs = Date.now() + input.durationHours * 3_600_000
	}
...
function parseLimitedOffer(input: any = {}) {
...
export async function sendInteractiveMessage(sock: any, jid: string, content: any = {}, options: any = {}) {
...
export async function createCarouselCard(sock: any, content: any = {}) {
	...
	if (media) {
		try {
			const mediaContent = await prepareWAMessageMedia(media, { upload: sock.waUploadToServer } as any)
...
export async function sendCarouselMessage(sock: any, jid: string, content: any = {}, options: any = {}) {
...
export class InteractiveBuilder {
	...
	offer(...args: any[]) {
		...
		if (expOption) {
			...
			if (opt === 'day') expiryTime = now + 86400000
			else if (opt === 'week') expiryTime = now + 86400000 * 7
			else if (opt === 'month') expiryTime = now + 86400000 * 30
			else if (opt === 'year') expiryTime = now + 86400000 * 365
			else if (opt === 'exp') expiryTime = now - 60000
		}
	...
	bottomsheet(title: string, buttonText: string, inThreadButtonsLimit: number = 1, dividerIndices: number[] = [1, 2]) {
		this.#bottomSheet = {
			list_title: title,
			button_title: buttonText,
			in_thread_buttons_limit: inThreadButtonsLimit,
	...
	setContextInfo(ci: any) {
		this.#contextInfo = ci
		return this
	}

	#parseBtnArgs(args: any[], type: string) {
		let text = ''
		let value = ''
		let icon = ''
	...
	qreplybtn(...args: any[]) {
		const { text, value, icon } = this.#parseBtnArgs(args, 'reply')
		this.#buttons.push({ type: 'reply', text, value, ...(icon ? { icon } : {}) })
		return this
	}

	urlbtn(...args: any[]) {
		const { text, value, icon } = this.#parseBtnArgs(args, 'url')
		this.#buttons.push({ type: 'url', text, value, ...(icon ? { icon } : {}) })
		return this
	}

	copybtn(...args: any[]) {
		const { text, value, icon } = this.#parseBtnArgs(args, 'copy')
		this.#buttons.push({ type: 'copy', text, value, ...(icon ? { icon } : {}) })
		return this
	}

	callbtn(...args: any[]) {
		const { text, value, icon } = this.#parseBtnArgs(args, 'call')
		this.#buttons.push({ type: 'call', text, value, ...(icon ? { icon } : {}) })
		return this
	}
	...
	bookingbtn(...args: any[]) {
		...
		if (typeof args[0] === 'object' && args[0] !== null) {
			...
			const parsed = this.#parseBtnArgs([args[0]], 'booking')
			text = parsed.text
			config = args[1] || {}
		}
		this.#buttons.push({
	...
	galaxybtn(text: string, screen: string, data: any = {}, flowVersion: string = '3', flowAction: string = 'navigate') {
		this.#buttons.push({
			type: 'native',
			text,
	...
	listbtn(...args: any[]) {
		const { text, value, icon, rows } = this.#parseBtnArgs(args, 'list')
		this.#buttons.push({ type: 'single_select', text: text || value, rows, ...(icon ? { icon } : {}) })
		return this
	}

	async send(from: string, options: any = {}) {
		const params = { ...(options.params || {}) }
		if (this.#bottomSheet) {
			params.bottom_sheet = this.#bottomSheet
		}
		if (this.#offer) {
			params.limited_time_offer = this.#offer
		}

		return sendInteractiveMessage(this.#sock, from, {
			title: this.#title,
			body: this.#body,
			footer: this.#footer,
			buttons: this.#buttons,
			params,
			contextInfo: this.#contextInfo || options.contextInfo
		})
	}
}
...
export class CarouselCardBuilder {
	...
	async build() {
		...
		return createCarouselCard(this.#sock, {
...
export function patchSocketInteractive(sock: any): any {
...
const EWALLET_PRESETS: Record<string, any> = {
	dana: {
		key: '087873384161',
		name: 'DANA',
...
const EWALLET_ALIASES: Record<string, string> = {
	shopepay: 'seabank',
	qris: 'seabank',
	sea: 'seabank',
	seabank: 'seabank'
...
function makeId(prefix: string = 'PAY') {
	return `${prefix}-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`
}

function asText(value: any = 'tes') {
	const text = String(value ?? '').trim()
	return text || 'tes'
}

function asNumber(value: any, fallback: number = 0) {
	const number = Number(value)
	return Number.isFinite(number) ? number : fallback
}

function money(value: any = DEFAULT_AMOUNT.value, offset: any = DEFAULT_AMOUNT.offset) {
	return {
...
function zeroMoney(offset: any = 100) {
	return {
		value: 0,
...
function clonePayment(value: any): any {
	if (Array.isArray(value)) return value.map(clonePayment)
	if (!value || typeof value !== 'object') return value
	const out: any = {}
	for (const [key, item] of Object.entries(value)) out[key] = clonePayment(item)
	return out
}
...
function defaultEwalletParams(profile: any = EWALLET_PRESETS.dana) {
	return {
		currency: 'IDR',
...
class BaseNativeBuilder {
	...
	title(text: string = 'tes') {
		this.state.header.title = asText(text)
		return this
	}

	body(text: string = 'tes') {
		this.state.body.text = String(text ?? 'tes')
		return this
	}

	text(text: string = 'tes') {
		return this.body(text)
	}

	footer(text: string = 'tes') {
		this.state.footer.text = String(text ?? 'tes')
		return this
	}

	button(name: string = this.state.buttonName) {
		...
		try {
			...
			if (this.state.media && this.sock.waUploadToServer) {
				try {
					...
					if (!(header as any).imageMessage && (header as any).jpegThumbnail) {
						const jpegThumbnail = (header as any).jpegThumbnail
						delete (header as any).jpegThumbnail
						;(header as any).imageMessage = { jpegThumbnail }
					}
				}
				delete header.title
			} else if ((header as any).jpegThumbnail) {
				const jpegThumbnail = (header as any).jpegThumbnail
				delete (header as any).jpegThumbnail
				;(header as any).imageMessage = { jpegThumbnail }
			}

			delete (header as any).hasMediaAttachment
			delete (header as any).jpegThumbnail
...
export class PaymentBuilder extends BaseNativeBuilder {
	...
	orderStatus(value: string = 'shipped') {
		this.state.params.order.status = asText(value)
		return this
	}

	orderDescription(value: string = 'tes') {
		this.state.params.order.description = asText(value)
		return this
	}

	orderType(value: string = 'ORDER') {
		this.state.params.order.order_type = asText(value)
		return this
	}

	subtotal(value: any = DEFAULT_AMOUNT.value, offset: any = DEFAULT_AMOUNT.offset) {
		this.state.params.order.subtotal = money(value, offset)
		return this
	}

	tax(value: any = 0, offset: any = 100) {
		this.state.params.order.tax = money(value, offset)
		return this
	}

	discount(value: any = 0, offset: any = 100) {
		this.state.params.order.discount = money(value, offset)
		return this
	}

	shipping(value: any = 0, offset: any = 100) {
	...
	item(value: any = {}) {
		const item: any = {
			...
			amount:
				value.amount && typeof value.amount === 'object'
					? value.amount
					: money(value.value ?? DEFAULT_AMOUNT.value, value.offset ?? DEFAULT_AMOUNT.offset),
			quantity: asNumber(value.quantity, 1)
		}

		if (value.sale_amount) item.sale_amount = value.sale_amount
		if (value.description) item.description = asText(value.description)
	...
	nativePaymentMethods(values: any[] = []) {
		this.state.params.native_payment_methods = values.map((item: any) =>
			typeof item === 'string' ? item : JSON.stringify(item)
		)
		return this
	}

	paymentMethods(values: any[] = []) {
		return this.nativePaymentMethods(values)
	}

	shareStatus(value: boolean = true) {
		this.state.params.share_payment_status = Boolean(value)
		return this
	}

	softDeleted(value: boolean = false) {
		this.state.params.is_soft_deleted = Boolean(value)
		return this
	}

	template(value: any = {}) {
		if (value.header) this.header(value.header)
		if (value.body !== undefined) this.body(value.body)
		if (value.footer !== undefined) this.footer(value.footer)
		if (value.buttonName) this.button(value.buttonName)
		if (value.params) this.params(value.params)
		if (value.messageParams !== undefined) this.messageParams(value.messageParams)
		return this
	}
}

export class EWalletBuilder extends BaseNativeBuilder {
	walletName: string = 'dana'
	...
	amount(value: any = 0, offset: any = 100) {
		this.state.params.total_amount = money(value, offset)
		this.state.params.order.subtotal = money(value, offset)
	...
	reference(value: string = makeId('EWALLET')) {
		this.state.params.reference_id = asText(value)
		return this
	}

	type(value: string = 'physical-goods') {
		this.state.params.type = asText(value)
		return this
	}
	...
	orderStatus(value: string = 'pending') {
		this.state.params.order.status = asText(value)
		return this
	}
		...
		this.state.params.order.items = [
			{
				...
				sale_amount:
					value.sale_amount && typeof value.sale_amount === 'object'
						? value.sale_amount
	...
	referral(value: string = 'chat_attachment') {
		this.state.params.referral = asText(value)
		return this
	}

	template(value: any = {}) {
		if (value.wallet) this.wallet(value.wallet)
		if (value.header) this.header(value.header)
		if (value.title) this.title(value.title)
		if (value.body !== undefined) this.body(value.body)
		if (value.footer !== undefined) this.footer(value.footer)
		if (value.params) this.params(value.params)
		if (value.paymentKey) this.paymentKey(value.paymentKey)
		if (value.messageParams !== undefined) this.messageParams(value.messageParams)
		return this
	}
...
export class OrderBuilder {
	...
	orderId(val: string = `INV-${Date.now()}`) {
		this.#state.orderId = String(val)
		return this
	}

	thumbnail(val: string = TINY_JPEG) {
		this.#state.thumbnail = String(val ?? TINY_JPEG)
		return this
	}

	async thumbnailFromProfile(jid: string) {
		try {
			const sharpMod = (await import('sharp')).default
			const imageUrl = await this.#sock.profilePictureUrl(jid, 'image').catch(() => null)
			...
			this.#state.thumbnail = thumbnailBuffer.toString('base64')
		} catch (e: any) {
	...
	itemCount(val: number = 1) {
		this.#state.itemCount = Math.max(0, Number(val) || 0)
		return this
	}

	status(val: number = ORDER_STATUS.CONFIRMED) {
		this.#state.status = Number(val)
		return this
	}

	surface(val: number = ORDER_SURFACE.BILLING) {
		this.#state.surface = Number(val)
		return this
	}

	message(val: string = ZERO_WIDTH_FILL) {
		this.#state.message = val !== undefined && val !== null ? String(val) : ''
		return this
	}

	orderTitle(val: string = '') {
		this.#state.orderTitle = String(val ?? '')
		return this
	}

	sellerJid(val: string = '') {
		this.#state.sellerJid = String(val ?? '')
		return this
	}

	token(val: string = `INV-TOKEN-${Date.now()}`) {
		this.#state.token = String(val)
		return this
	}

	totalAmount(val: number = 0, divisor: number = 1000) {
		this.#state.totalAmount1000 = Math.round(Number(val) / divisor)
		return this
	}

	totalAmountRaw(val: number = 0) {
		this.#state.totalAmount1000 = Number(val) || 0
		return this
	}

	currency(val: string = 'IDR') {
		this.#state.totalCurrencyCode = String(val)
		return this
	}

	contextInfo(info: any = null) {
	...
	mentionedJid(...jids: any[]) {
		const arr = jids
			.flat()
			.filter(Boolean)
			.map((j: any) => (String(j).includes('@') ? j : `${j}@s.whatsapp.net`))
		if (arr.length > 0) {
			if (!this.#state.contextInfo)
				this.#state.contextInfo = { mentionedJid: [], groupMentions: [], statusAttributions: [] }
			this.#state.contextInfo.mentionedJid = arr
		}
		return this
	}

	senderKeyDistribution(groupId: string, axolotlKey: string) {
		if (groupId && axolotlKey) {
			this.#state.senderKeyDistribution = {
	...
	noSenderKeyDistribution() {
		this.#state.senderKeyDistribution = null
		return this
	}

	template(value: any = {}) {
		if (value.orderId) this.orderId(value.orderId)
		if (value.thumbnail) this.thumbnail(value.thumbnail)
		if (value.itemCount !== undefined) this.itemCount(value.itemCount)
		if (value.status !== undefined) this.status(value.status)
		if (value.surface !== undefined) this.surface(value.surface)
		if (value.message !== undefined) this.message(value.message)
		if (value.orderTitle) this.orderTitle(value.orderTitle)
		if (value.sellerJid) this.sellerJid(value.sellerJid)
		if (value.token) this.token(value.token)
		if (value.totalAmount !== undefined) this.totalAmountRaw(value.totalAmount)
		if (value.currency) this.currency(value.currency)
		if (value.contextInfo) this.contextInfo(value.contextInfo)
	...
	build() {
		if (!this.#state.orderTitle) {
			throw new Error('orderTitle harus diisi')
		}
		if (!this.#state.sellerJid) {
		...
		const orderMessage: any = {
			...
			sellerJid: this.#state.sellerJid,
			token: this.#state.token,
			totalAmount1000: this.#state.totalAmount1000,
			totalCurrencyCode: this.#state.totalCurrencyCode
		}

		if (this.#state.contextInfo) {
			orderMessage.contextInfo = this.#state.contextInfo
		}

		const msg: any = { orderMessage }
		if (this.#state.senderKeyDistribution) {
			msg.senderKeyDistributionMessage = this.#state.senderKeyDistribution
		}
	...
	async send(jid: string, options: any = {}) {
		...
		return this.#sock.relayMessage(jid, msg, options)
	}
}

export function createPaymentBuilder(sock: any) {
	return new PaymentBuilder(sock)
}

export function createEWalletBuilder(sock: any, wallet: string = 'dana') {
	return new EWalletBuilder(sock).wallet(wallet)
}

export function createOrderBuilder(sock: any) {
	return new OrderBuilder(sock)
}

export function usePayment(sock: any): any {
	const version = 2
	if (sock.__paymentBuilderVersion === version) return sock

	sock.payment = () => createPaymentBuilder(sock)
	sock.ewallet = (wallet: string = 'dana') => createEWalletBuilder(sock, wallet)
	sock.paymentKey = (wallet: string = 'dana') => createEWalletBuilder(sock, wallet)
...
export class LinkPreviewBuilder {
	...
	type(val: string = 'small') {
		const t = String(val || 'small').toLowerCase()
		this.#previewType = t === 'big' ? 'big' : 'small'
		return this
	}
	...
	async send(jid: string, options: any = {}) {
		...
		const link = this.#link || 'https://example.com'
		const title = this.#title || 'Link Preview'
		const description = this.#description || '© powered by bot'
		const text = this.#text.includes(link) ? this.#text : `${link}\n${this.#text || title}`

		let buffer: Buffer | null = null
		if (this.#imageInput) {
			if (Buffer.isBuffer(this.#imageInput)) {
				buffer = this.#imageInput
			} else if (typeof this.#imageInput === 'string') {
				try {
					const res = await fetch(this.#imageInput)
					if (res.ok) buffer = Buffer.from(await res.arrayBuffer())
				} catch {}
			}
		}

		if (this.#previewType === 'big' && buffer) {
			const tempPath = path.join(
				os.tmpdir(),
				`lp-hq-${Date.now()}-${crypto.randomBytes(4).toString('hex')}.png`
			)
			try {
				fs.writeFileSync(tempPath, buffer)
				const { imageMessage } = await prepareWAMessageMedia(
					{ image: { url: tempPath } } as any,
					{
						upload: this.#sock.waUploadToServer,
				...
				return await this.#sock.sendMessage(
					jid,
					{
						...
						linkPreview: {
							...
							title,
							description,
							jpegThumbnail: (imageMessage as any).jpegThumbnail,
...
export function createLinkPreviewBuilder(sock: any): LinkPreviewBuilder {
	return new LinkPreviewBuilder(sock)
}

export const patchLinkPreview = (sock: any) => {
	if (!sock || sock.__linkPreviewPatched) return sock
...
export function applyNaileys(sock: any): void {
